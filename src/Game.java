...
public class Game extends Canvas {
    ...
    private static void run() {
        ...
        while (!Board.isGameOver(board, length)) {
            ...
            if (human == player) {
                ...
                int[] play = AI(board, computer, human, difficulty_input, length);
                int row = play[0];
                int col = play[1];
                Board.play(board, row, col, computer);
        ...
        } else if (Board.won(board, O, length)) {
            draw_winner_piece = O;
            if (human == O) {
                draw_end_game_type = 1;
            } else {
    ...
    private static int inputWinLength(int[][] board) {
        ...
        do {
            System.out.printf("Pick a win_length in [%d,%d]: ", MIN_BOARD_SIZE, max);
            input_length = scanner.nextLine().trim();
        } while (isInputInvalid(input_length, MIN_BOARD_SIZE, max));
    ...
    private static int inputDifficulty(int[][] board) {
        ...
        if (board.length == MIN_BOARD_SIZE && board[0].length == MIN_BOARD_SIZE) {
            ...
            } while (isInputInvalid(difficulty_string, MIN_AI, MAX_AI_3X3));
        }
    ...
    private static void inputScannerNextPlay(int[][] board, int human) {
        while (true) {
            ...
            while (input_row_string == null || isInputInvalid(input_row_string, 0, board.length - 1)) {
            ...
            if (Board.canPlay(board, row, col)) {
                Board.play(board, row, col, human);
                return;
            } else {
    ...
    private static int[] AI(int[][] original_board, int player1, int player2, int level, int length) {
        ...
        //Otherwise, multiply level by two(to get plays by each side for each level)
            ...
            for (Integer[] move : moves) {
                if (Board.canPlay(board, move[0], move[1])) {
    ...
    private static Integer[] minimax(int[][] board, int player1, int player2, int player, int depth, int max_depth, int length) {
        ...
        for (Integer[] move : moves) {
            ...
            int col = move[1];
            //Make play
            Board.play(board, row, col, player);
            ...
            score[1] = col;
            //Undo the play
            Board.play(board, row, col, EMPTY);
